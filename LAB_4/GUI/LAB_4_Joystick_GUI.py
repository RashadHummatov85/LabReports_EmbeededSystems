...
class PlotBox(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # "Data Visualization" text if empty
...
class MainWindow(QWidget):
    ...
    def toggle_run(self):
        if not self.running:
            ...
            self.startStopBtn.setText("Stop Test")
    ...
    def tick(self):
        ...
        try:
            ...
            now = time.time()
            dt = now - (self.last_ts or now)
            self.last_ts = now
            if dt > 0:
                self.hz = 0.9 * self.hz + 0.1 * (1.0 / dt) if self.hz else (1.0 / dt)
                self.rateText.setText(f"Sample Rate: {self.hz:0.1f} Hz")

            # Convert to voltage (0..5V)
            vx = x * 5.0 / 1023.0
            vy = y * 5.0 / 1023.0
            self.xText.setText(f"X: {vx:0.2f} V  ({x})")
            self.yText.setText(f"Y: {vy:0.2f} V  ({y})")
            self.btnText.setText("Button: PRESSED" if pressed == 1 else "Button: released")

            # Update plot
            self.plot.push(x, y)

            # Direction mapping (deadzone around center)
            dx = x - 512
            dy = y - 512
            dead = 70
            direction = "CENTER"
            if abs(dx) > dead or abs(dy) > dead:
                if abs(dx) > abs(dy):
                    ...
                    direction = "UP" if dy < 0 else "DOWN"  # note: many joysticks invert Y
            self.mapping.set_dir(direction)

        except Exception:
            # If COM port disconnects, stop cleanly
            self.toggle_run()
